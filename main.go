...
// makePizza attemps to make pizza. We generate a random number from 1-12,
...
// will take a differente length of time to produce (some pizzas are harder than others).
...
// sends it (when te number of orders is greater than or equal to the
...
func pizzeria(pizzaMaker *Producer) {
	...
	// until de quit channel recieve something.
	for {
		currentPizza := makePizza(i)
		if currentPizza != nil {
			i = currentPizza.pizzaNumber

			// select stmnt are only useful for channels!!!
			select {
